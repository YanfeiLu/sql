...
SELECT *,
COUNT(product_id) OVER (PARTITION BY customer_id, product_id) AS total_purchases_of_the_product
FROM customer_purchases;
...
CREATE TEMP TABLE IF NOT EXISTS temp.sales_by_date AS

SELECT market_date, SUM(quantity * cost_to_customer_per_qty) AS total_sales
,ROW_NUMBER() OVER(ORDER BY SUM(quantity * cost_to_customer_per_qty) DESC) AS sales_rank
FROM customer_purchases
...
SELECT 
    market_date,
    total_sales,
    'worst_day' AS comparison
FROM temp.sales_by_date
WHERE sales_rank = (SELECT MAX(sales_rank) FROM temp.sales_by_date);
...
SELECT
    v.vendor_name,
    p.product_name,
    5 * vi.original_price * c.customer_count AS income_per_product
